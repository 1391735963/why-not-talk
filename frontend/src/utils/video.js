...
import { socket, breakVideoConnect } from "./socket";
import { storeToRefs } from "pinia";
const store = storeIndex();
const { chatBox, login, videoChat } = storeToRefs(store);
var stream,
  peer,
  isCalling = false; //初始化要发送的流,和描述文件,通话状态
function startVideoStream(e) {
  //开始传输视频流
  createMedia();
}

function stopVideoStream(data) {
  //停止传输视频流
